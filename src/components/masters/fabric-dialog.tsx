...
import { MasterFabric } from "@/data/mock-masters";

interface FabricDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialData: MasterFabric | null;
  onSave: (data: MasterFabric) => void;
}

export function FabricDialog({ open, onOpenChange, initialData, onSave }: FabricDialogProps) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm<MasterFabric>({
    defaultValues: {
      id: "",
      name: "",
    }
  ...
  const onSubmit = (data: MasterFabric) => {
    onSave(data);
  };
      ...
      <DialogContent className="sm:max-w-[425px] p-0 overflow-hidden bg-white shadow-2xl border-0">
        <DialogHeader className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <DialogTitle className="text-xl font-bold text-slate-900">
            {initialData ? 'Edit Fabric' : 'Add Fabric'}
          </DialogTitle>
        </DialogHeader>
