...
export function SelectSalesOrderItemsDialog({
  ...
  const renderRow = (item: typeof soItems[0], isLocked: boolean) => {
    ...
    const reqQty = type === "Fabric" ? (item as any).requiredQtyMtr : totalPcs;
    const alreadyOrdered = Math.floor(reqQty * 0.5); // Mock
    const balanceRequired = reqQty - alreadyOrdered;

    return (
      <tr
        key={item.id}
        className={`${isLocked ? 'bg-slate-50/70 cursor-not-allowed' : 'hover:bg-blue-50/50 cursor-pointer transition-colors'} ${selectedItemIds.includes(item.id) ? 'bg-blue-50/50' : ''}`}
      ...
      >
        ...
        <td className="px-4 py-4">
          <div className={`font-bold ${isLocked ? 'text-slate-500' : 'text-[#0453B8]'}`}>{item.soNo}</div>
          <div className={`text-xs font-medium mt-0.5 ${isLocked ? 'text-slate-400' : 'text-slate-500'}`}>Line {item.soItem.split('-')[1]}</div>
        </td>
        <td className="px-4 py-4">
          <div className={`font-bold ${isLocked ? 'text-slate-500' : 'text-slate-800'}`}>{item.productId}</div>
          <div className={`text-xs font-medium mt-0.5 ${isLocked ? 'text-slate-400' : 'text-slate-500'}`}>{item.name}</div>
          {type === "Fabric" && (
            <div className={`text-[10px] font-medium mt-1 uppercase tracking-wide ${isLocked ? 'text-slate-400' : 'text-slate-500'}`}>
              Fabric: {item.fabricBom?.type || 'Cotton'} • {item.fabricBom?.gsm || '180'} GSM • {item.fabricBom?.width || '44"'} W
            </div>
          )}
        </td>
        <td className="px-4 py-4">
          <div className="flex items-center gap-4">
            <div className={`w-[60px] h-[75px] flex items-center justify-center overflow-hidden shrink-0 border border-slate-200 rounded ${isLocked ? 'opacity-50 grayscale' : ''} bg-slate-50`}>
              <img 
                src={
            ...
            <div className="flex flex-col justify-center">
              <div className={`font-bold ${isLocked ? 'text-slate-500' : 'text-slate-800'}`}>{item.color}</div>
              {type === "Fabric" && (
                <div className={`text-[10px] font-medium mt-0.5 ${isLocked ? 'text-slate-400' : 'text-slate-500'}`}>
                  Color: {item.fabricBom?.color || item.color}
                </div>
              )}
            </div>
          </div>
        ...
        <td className="px-4 py-4 text-right">
          <div className={`font-bold ${isLocked ? 'text-slate-500' : 'text-slate-800'}`}>{reqQty.toFixed(type === "Fabric" ? 2 : 0)}</div>
          <div className={`text-xs font-medium mt-0.5 ${isLocked ? 'text-slate-400' : 'text-slate-500'}`}>{alreadyOrdered.toFixed(type === "Fabric" ? 2 : 0)}</div>
        </td>
        <td className="px-4 py-4 text-right">
          <div className={`font-bold ${isLocked ? 'text-slate-500' : 'text-slate-900'}`}>{balanceRequired.toFixed(type === "Fabric" ? 2 : 0)}</div>
        </td>
      </tr>
    );
  };
      ...
      <DialogContent className="w-[95vw] sm:max-w-[1000px] h-[90vh] sm:h-[750px] flex flex-col p-0 overflow-hidden bg-white [&>button]:hidden shadow-2xl">
                ...
                <tr>
                  <th className="px-4 py-3 text-center w-12">
                    <input 
                      ...
                      className="rounded border-slate-300 text-[#0453B8] focus:ring-[#0453B8] w-4 h-4 cursor-pointer accent-[#0453B8]"
                    />
                  </th>
                  ...
                  <th className="px-4 py-3 font-bold text-slate-700">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-slate-500 font-medium">Item & Fabric Info</span>
                    </div>
                  </th>
                  <th className="px-4 py-3 font-bold text-slate-700">
                    <div className="flex flex-col">
                  ...
                  <th className="px-4 py-3 font-bold text-slate-700 text-right">
                    <div className="flex flex-col items-end">
                      <span>Required Qty ({type === "Fabric" ? "Mtr" : "Pcs"})</span>
                      <span className="text-xs text-slate-500 font-medium">Already Ordered</span>
                    </div>
                  </th>
                  <th className="px-4 py-3 font-bold text-slate-700 text-right">
                    <div className="flex flex-col items-end">
                      <span>Balance Required ({type === "Fabric" ? "Mtr" : "Pcs"})</span>
                    </div>
                  </th>
                </tr>
              </thead>
        ...
        <div className="px-6 py-4 bg-white border-t border-slate-200 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-8 text-sm">
            <span className="font-bold text-slate-800">{selectedItemIds.length} lines selected</span>
            <span className="font-bold text-[#0453B8]">Total Required: {totalRequired.toFixed(2)} {type === "Fabric" ? "Mtr" : "Pcs"}</span>
          </div>
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={handleNext}
              className="bg-[#0453B8] hover:bg-blue-700 text-white font-bold px-6 shadow-sm disabled:opacity-50"
            >
              Add Selected Lines
            </Button>
          </div>
