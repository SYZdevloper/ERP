...
export default function PrintingJobCardView() {
  ...
  const handleComplete = () => {
    setIsSaved(true);
    // Transition to the next phase: Finishing
    updateJobCard(id, { currentPhase: "Finishing" });
    setTimeout(() => {
    ...
    <div className="flex flex-col h-full bg-slate-50/50 p-6 overflow-y-auto">
      <div className="flex items-center justify-between mb-6 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 mb-1">Printing: {id}</h1>
        ...
        <div>
          ...
          >
            {isSaved ? (
              <><CheckCircle2 className="w-4 h-4 mr-2" /> Passed to Finishing</>
            ) : (
              <><Save className="w-4 h-4 mr-2" /> Complete & Send to Finishing</>
            )}
          </Button>
        </div>
      </div>
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 text-slate-600">
        Placeholder for Printing details and data entry.
      </div>
    </div>
  );
}
