...
export function PendingFabricPOs() {
  const [searchQuery, setSearchQuery] = useState("");

  const filtered = MOCK_PENDING_POS.filter(
    (item) =>
  ...
  const calculateDelay = (soDateStr: string) => {
    // Current date is conceptually Aug 7 or 8 based on context, let's use actual current date
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex flex-col flex-1 overflow-hidden mt-2">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <div className="relative w-72">
            ...
            <Input
              placeholder="Search by SO, Client or Material..."
          ...
          <Table>
            <TableHeader className="bg-slate-50 sticky top-0 z-10 shadow-sm">
              <TableRow>
                ...
                <TableHead className="px-6 py-3 w-[20%]">Required Material</TableHead>
                  ...
                  return (
                    <TableRow key={item.id} className="hover:bg-slate-50/50">
                      ...
                      <TableCell className="px-6 py-3 text-center">
                        <Link href={`/fabric-purchases/create?so=${item.id}`}>
